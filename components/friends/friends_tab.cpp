	...
	else
	{
		...
		else
		{
			...
			if (Button((string(ICON_OPEN_LINK) + "Open Profile").c_str()))
			{
				if (D.id != 0)
				{
					string url = "https://www.roblox.com/users/" + to_string(D.id) + "/profile";
					ShellExecuteA(nullptr, "open", url.c_str(), nullptr, nullptr, SW_SHOWNORMAL);
					LOG_INFO("Opened profile in browser.");
				}
				else { LOG_INFO("No friend selected or friend ID is invalid."); }
			}
			SameLine();
			if (Button((string(ICON_INVENTORY) + "Open Inventory").c_str()))
			{
				if (D.id != 0)
				{
					string url = "https://www.roblox.com/users/" + to_string(D.id) + "/inventory/#!/accessories";
					ShellExecuteA(nullptr, "open", url.c_str(), nullptr, nullptr, SW_SHOWNORMAL);
					LOG_INFO("Opened inventory in browser.");
				}
				else { LOG_INFO("No friend selected or friend ID is invalid."); }
			}
