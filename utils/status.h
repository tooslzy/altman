...
#include <chrono>
#include <iostream>
// #include "../components/console/console.h" // No longer needed for Status::Get
...
namespace Status
{
	...
	inline void Set(const string& s)
	{
		...
		thread([tp, s]()
		{
			for (int i = 5; i >= 0; --i) // Loop from 5 down to 0
			{
				...
				{
					// Another Status::Set call has taken over
					return;
				}
			}
		}).detach();
	...
	{
		lock_guard<mutex> lock(_mtx);
		return _displayText; // Return the text with the countdown
	}
}
